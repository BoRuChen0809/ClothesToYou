...
from user.modules.get_probable_images_vgg16 import vgg16_get_probable_images
...
if import_method == "v0":
    ...
    from user.modules.v2_module import build_resnet_model
...
class ReverseImageSearch:
    def __init__(self, method, MAX_AMT, top_K, raw_img, img_db_path):
        ...
        #☆☆☆
        #img_db_path = "D:/MyPrograms/Clothes2U/DB/訓練圖片 DB/origin/dataset/img_db_3"
        ...
        #raw_img = "D:/MyPrograms/Clothes2U/DB/服飾圖片 DB/Lativ/media2/products/WOMEN/上衣類/162305980010/162305980010135.jpg"
        #raw_img = "D:/MyPrograms/Clothes2U/DB/訓練圖片 DB/origin/dataset/test_data_to_classify/2/GU_咖啡色_洋裝類.jpg"
        #raw_img = "D:/MyPrograms/Clothes2U/DB/訓練圖片 DB/origin/dataset/test_data_to_classify/2/GU_白色_童裝類_Angela Mckay x H&M.jpg"
        #raw_img = "D:/MyPrograms/Clothes2U/DB/訓練圖片 DB/origin/dataset/test_data_to_classify/2/UQ_黃色_襯衫類_POLO衫.jpg"
        #raw_img = "D:/MyPrograms/Clothes2U/DB/訓練圖片 DB/origin/dataset/img_db_3/000102_灰色_洋裝類/00000000 (21).jpg"
    ...
    def exec_reverse_image_search(self):
        if self.available:
            #self.import_modules()
            start_time = time()
            querying_cv_img = self.preprocess_image(self.raw_img)
            
            ''' Get all images and build a model '''
            probable_img_list = vgg16_get_probable_images(self.raw_img, self.img_db_path)
            ...
            print(f"圖片搜尋已完成，花費時間: {time_consumed} 秒")
            ...
            ''' Return similar image paths '''
            return [probable_img_list[img_idx] for img_idx in result[0]]
...
if __name__ == "__main__":
    ...
    Time consumed: 8.02 seconds  <=  K = 50
    ----------------
    (not website) K = 30, 圖片搜尋已完成，花費時間: 22.43 秒
    (not website) K = 40, 圖片搜尋已完成，花費時間: 24.62 秒
    (not website) K = 50, 圖片搜尋已完成，花費時間: 20.94 秒
    '''
    raw_img = "D:/MyPrograms/Clothes2U/DB/訓練圖片 DB/origin/dataset/test_data_to_classify/2/GU_咖啡色_洋裝類.jpg"
    img_db_path = "D:/MyPrograms/Clothes2U/DB/訓練圖片 DB/origin/dataset/img_db_3" 
    
    ris = ReverseImageSearch(method, MAX_AMT, top_K, raw_img, img_db_path)
